...
class App(tk.Frame):
    ...
    def new_game(self):
        ...
        player1_var.trace("w",
                          lambda *args: listbox_player1_depth.config(
                              state=tk.NORMAL if player1_var.get() == "Agent" else tk.DISABLED)
                          )
        player2_var.trace("w",
                          lambda *args: listbox_player2_depth.config(
                              state=tk.NORMAL if player2_var.get() == "Agent" else tk.DISABLED)
                          )
        ...
        self.new_game_window.geometry(
            "{}x{}+{}+{}".format(w, h, int(win_x + (win_w - w) * 0.5), int(win_y + (win_h - h) * 0.5)))
    ...
    def run_game(self, player1, search_depth1, player2, search_depth2, search_fn):
        ...
        def place(player, col, render=True):
            if self.board.place(player, col):
                ...
                self.canvas.create_text(board_pos[0] + (col + 0.5) * self.cell_size,
                                        board_pos[1] + (row + 0.5) * self.cell_size,
                                        text="{}".format(self.placement_counter[player]), fill="black", font=(None, 10),
                                        anchor="center"
                                        )
                self.canvas.update()
